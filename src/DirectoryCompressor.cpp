...
void DirectoryCompressor::unpackArchive(const fs::path &archive_path) {
...
void DirectoryCompressor::compressDirectory(const fs::path &dir_to_compress, std::ofstream &new_archive,
    ...
    for (const auto &dir_entry: fs::directory_iterator(dir_to_compress)) {
        ...
        if (fs::is_directory(current)) {
            compressDirectory(current, new_archive, new_relative);
        } else if (fs::is_regular_file(current)) {
            compressFile(current, new_archive, new_relative);
        }
    }
}

void DirectoryCompressor::addDirectory(std::ofstream &new_archive, const fs::path &relative_path) {
